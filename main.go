...
func init() {
	...
	privateBytes, err := ioutil.ReadFile("rsa")
	if err != nil {
		log.Fatal(err)
	}
	key, err = x509.ParsePKCS1PrivateKey(privateBytes)
	if err != nil {
		log.Fatal(err)
	}

	publicBytes, err := ioutil.ReadFile("rsa.pub")
	if err != nil {
		log.Fatal(err)
	}
	pubKey, err := x509.ParsePKCS1PublicKey(publicBytes)
	if err != nil {
		log.Fatal(err)
	}
...
func completeTask(text string) error {
	filter := bson.D{primitive.E{Key: "text", Value: encrypt(text)}}
...
func decrypt(cipherText string) string {
	object, err := jose.ParseSigned(cipherText)
	if err != nil {
		panic(err)
	}

	output, err := object.Verify(&key.PublicKey)
	if err != nil {
		panic(err)
	}
